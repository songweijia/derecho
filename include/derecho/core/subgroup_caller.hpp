...
private:
    ...
    /** The shard num of this node belongs to. */
    subgroup_id_t shard_num;
...
public:
    SubgroupCaller(uint32_t type_id, node_id_t nid, subgroup_id_t subgroup_id, uint32_t shard_num, rpc::RPCManager& group_rpc_manager);
     ...
     * @param dest_node_id The ID of the node that the P2P message should be sent to
     ...
     */
    template <rpc::FunctionTag tag, typename... Args>
    auto ordered_send(NodeId dest_node_id, Args&&... args);
     ...
     */
    template <rpc::FunctionTag tag, typename... Args>
    auto ordered_send(ShardNum dest_shard_num, Args&&... args);
