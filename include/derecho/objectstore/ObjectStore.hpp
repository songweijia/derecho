...
public:
    ...
    virtual std::tuple<version_t,uint64_t> put(const Object& object, const bool &force_client = false) = 0;
    // 2 - blocking remove
    // @PARAM oid - const reference of the object id.
    // @PARAM force_client - see above
    // @RETURN version of this remove operation
    virtual std::tuple<version_t,uint64_t> remove(const OID& oid, const bool &force_client = false) = 0;
    // 3 - blocking get
    // @PARAM oid - const reference of the object id.
    ...
    virtual Object get(const OID& oid, const version_t& ver = INVALID_VERSION, const bool& force_client = false) = 0;
    // 3.1 - temporal get
    // @PARAM oid - const reference of the object id.
    // @PARAM ts_us - timestamp.
    // @RETURN the object of oid, invalid object if corresponding object does not exists.
    virtual Object get(const OID& oid, const uint64_t& ts_us) = 0;
